...
namespace D_Graves
{
    class Program
    {
        ...
        private static readonly int[] Gravesqew = { 1, 3, 1, 2, 1, 4, 1, 3, 1, 3, 4, 3, 2, 3, 2, 4, 2, 2 };
        private static readonly int[] Gravesqwe = { 1, 3, 2, 1, 1, 4, 1, 2, 1, 2, 4, 2, 2, 3, 3, 4, 3, 3 };
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            _config.SubMenu("Combo").AddItem(new MenuItem("qrange", "Your Q when Enemy range <").SetValue(new Slider(600, 450, 950)));
            ...
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != _player.Team))
                _config.SubMenu("R Factions").SubMenu("Use R combo").AddItem(new MenuItem("castR" + enemy.BaseSkinName, enemy.BaseSkinName).SetValue(true));
            ...
            _config.SubMenu("Farm").SubMenu("JungleClear").AddItem(new MenuItem("ActiveJungle", "Jungle key").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
            ...
            _config.SubMenu("Misc").AddItem(new MenuItem("gravesAutoLevel", "Auto Level")).SetValue(false);
            _config.SubMenu("Misc").AddItem(new MenuItem("gravesStyle", "Level Sequence").SetValue(
                new StringList(new[] { "Q-E-W", "Q-W-E" })));

            //HitChance
            _config.AddSubMenu(new Menu("HitChance", "HitChance"));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("Combo", "Combo"));
            _config.SubMenu("HitChance").SubMenu("Combo").AddItem(new MenuItem("Qchange", "Q Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").SubMenu("Combo").AddItem(new MenuItem("Wchange", "W Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").SubMenu("Combo").AddItem(new MenuItem("Rchange", "R Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("Harass", "Harass"));
            _config.SubMenu("HitChance").SubMenu("Harass").AddItem(new MenuItem("Qchangeharass", "Q Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").SubMenu("Harass").AddItem(new MenuItem("Wchangeharass", "W Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("KillSteal", "KillSteal"));
            _config.SubMenu("HitChance").SubMenu("KillSteal").AddItem(new MenuItem("Qchangekill", "Q Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").SubMenu("KillSteal").AddItem(new MenuItem("Wchangekill", "W Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            _config.SubMenu("HitChance").SubMenu("KillSteal").AddItem(new MenuItem("Rchangekill", "R Hit").SetValue(new StringList(new[] { "Low", "Medium", "High", "Very High" })));
            //Drawings
            ...
            _config.Item("gravesAutoLevel").ValueChanged += LevelUpMode;
            if (_config.Item("gravesAutoLevel").GetValue<bool>())
            {
                new AutoLevel(Style());
            }
        ...
        {
            switch (_config.Item("gravesStyle").GetValue<StringList>().SelectedIndex)
            {
                case 0:
                    return Gravesqew;
                case 1:
        ...
        private static void Combo()
        {
            ...
            if (useQ && _q.IsReady())
            {
                var t = TargetSelector.GetTarget(customq, TargetSelector.DamageType.Magical);
                if (t.IsValidTarget(_q.Range - 70) && _q.GetPrediction(t).Hitchance >= Qchange())
                    _q.Cast(t, false, true);
            }
            if (useW && _w.IsReady())
            {
                var t = TargetSelector.GetTarget(_w.Range, TargetSelector.DamageType.Magical);
                if (t.IsValidTarget(_w.Range) && _w.GetPrediction(t).Hitchance >= Wchange())
                    _w.Cast(t, false, true);
            }
            if (useE)
            {
                var t = TargetSelector.GetTarget(_q.Range + _e.Range, TargetSelector.DamageType.Magical);
                if (t != null)
                    Fuckinge(t);
            }
            if (_r.IsReady() && useRcombo)
            {
                ...
                if (t.IsValidTarget(_r.Range) && _config.Item("castR" + t.BaseSkinName) != null &&
                ...
                {
                    if (_q.IsReady() && (_q.GetDamage(t) > t.Health || _player.GetAutoAttackDamage(t, true) * 2 > t.Health)) return;
        ...
        private static void Harass()
        {
            ...
            {
                var t = TargetSelector.GetTarget(_q.Range, TargetSelector.DamageType.Magical);
                if (t.IsValidTarget(_q.Range - 70) && _q.GetPrediction(t).Hitchance >= Qchangeharass())
                    _q.Cast(t, false, true);
            }
            if (useW && _w.IsReady())
            {
                var t = TargetSelector.GetTarget(_w.Range, TargetSelector.DamageType.Magical);
                if (t.IsValidTarget(_w.Range) && _w.GetPrediction(t).Hitchance >= Wchangeharass())
                    _w.Cast(t, false, true);
            }
        }
            ...
            foreach (var hero in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy))
            {
                if (_q.IsReady() && _config.Item("UseQM").GetValue<bool>())
                {
                    if (_q.GetDamage(hero) > hero.Health && hero.IsValidTarget(_q.Range - 30) &&
                        _q.GetPrediction(hero).Hitchance >= Qchangekill())
                    {
                ...
                if (_w.IsReady() && _config.Item("UseWM").GetValue<bool>())
                {
                    if (_w.GetDamage(hero) > hero.Health && hero.IsValidTarget(_w.Range) &&
                        _q.GetPrediction(hero).Hitchance >= Wchangekill())
                    {
                ...
                {
                    if (_q.IsReady() && _q.GetDamage(hero) > hero.Health) return;
                    if (_config.Item("castRkill" + hero.BaseSkinName) != null &&
                        _config.Item("castRkill" + hero.BaseSkinName).GetValue<bool>() == true &&
                        !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("Undying Rage") &&
                        _r.GetDamage(hero) > hero.Health && _r.GetPrediction(hero).Hitchance >= Rchangekill())
                    {
