...
namespace D_Jarvan
{
    internal class Program
    {
        ...
        private static void Combo()
        {
            ...
            if (useR && _r.IsReady())
            {
                if (t.IsValidTarget(_r.Range) && !_haveulti)
            ...
            if (useW && _w.IsReady())
            {
                if (t.IsValidTarget(_w.Range))
                    Utility.DelayAction.Add(1000, () => _w.Cast());
            }
            if (useQ && _q.IsReady() && !_e.IsReady())
            {
                if (t.IsValidTarget(_q.Range) && _r.GetPrediction(t).Hitchance >= HitChance.High)
                    _q.Cast(t);
            }
        ...
        private static void KillSteal()
        {
            foreach (var hero in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy))
            {
                ...
                {
                    if (hero.IsValidTarget(_q.Range) &&
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Render.Circle.DrawCircle(ObjectManager.Player.Position, _q.Range + _r.Range,
                    System.Drawing.Color.GreenYellow);
            }
            if (_config.Item("DrawEQF").GetValue<bool>())
            {
                Render.Circle.DrawCircle(ObjectManager.Player.Position,
                    _q.Range + _config.Item("FlashDista").GetValue<Slider>().Value, System.Drawing.Color.GreenYellow);
            }
        }
    }
}
